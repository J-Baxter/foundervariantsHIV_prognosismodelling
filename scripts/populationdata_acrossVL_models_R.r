...
populationmodel_acrossVL_Env <- function(ncVals = round(c(10^2, 10^2.5)), PerVirionProbability = 4.715*1e-8, PropExposuresInfective = 0.029){
        require(dplyr)
        ...
        chronic_prob_fulldist = dbinom(0:nc,
         ...
         myfun0 <- function(g0, distr0){
                 return(g0 * distr0)
         }
         
         primary_prob_fulldist <- colSums(matrix(unlist(purrr::pmap(.l = list(as.list(gp), primary_prob_fulldist),
                                                                    .f = myfun0)), byrow= TRUE, nrow = length(primary_prob_fulldist)))
         
         preaids_prob_fulldist <- colSums(matrix(unlist(purrr::pmap(.l = list(as.list(ga), preaids_prob_fulldist),
                                                                    .f = myfun0)), byrow= TRUE, nrow = length(preaids_prob_fulldist)))
         ...
         probNVirionsTransmittedPerSexAct_PRIMARY[1] <- sum(unlist(purrr::map(.x = gc,
                                                                              .f = ~(.x*((1-f) + f*primary_prob_fulldist[1])) )))  
         
         probNVirionsTransmittedPerSexAct_PRIMARY[-1] <- colSums(matrix(unlist(purrr::map(.x = gc,
                                                                                          .f = ~(.x*f*primary_prob_fulldist[-1]))), byrow= TRUE, nrow = length(chronic_prob_fulldist)))
         probNVirionsTransmittedPerSexAct_PREAIDS[1] = sum(unlist(purrr::map(.x = gc,
                                                                             .f = ~(.x * ((1-f) + f*preaids_prob_fulldist[1])))))
         probNVirionsTransmittedPerSexAct_PREAIDS[-1] <- colSums(matrix(unlist(purrr::map(.x = gc,
