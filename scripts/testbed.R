...
test <- cbind.data.frame(sex = sample(c('MF', 'FM'),NPAIRS, rep = T), 
                         gud_ = sample(c('pos','neg'), NPAIRS,  prob= c(0.3,0.7), rep = T),
                         sub_ = sample(c('A', 'C', 'D', 'R'), NPAIRS, rep = T),
  ...
  ungroup() %>%
  group_by(couple) %>%
  group_modify(function(.x, .y) .x %>% mutate(log10spvl_mean = rnormt(1, c(2.6, 7.14), mu = 4.51, s = 0.79))) %>%
  ungroup() %>%
...
results <- test_dummy %>%
   mutate(log10spvl = log10spvl_mean - 0.15*sexM - 0.15*A_1 - 0.32*A_2 + 0.35*A_3 + 0.73*A_4 + 
