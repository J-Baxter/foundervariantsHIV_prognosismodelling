...
List SIR_cpp_model(NumericVector times,NumericVector state,
  ...
  // Declare Parms
  double cd4_gamma = parms["cd4_gamma"]; //daily thymic production of new cd4
  double cd8_gamma = parms["cd8_gamma"]; //daily thymic production of new cd8
  ...
  double cd4_theta = parms["cd4_theta"]; // average clearance rate in antigenic exposure model
  double cd8_theta = parms["cd8_theta"]; // average exposure rate in antigenic exposure model
  double e_cd4 = parms["e_cd4"];
  double e_cd8 = parms["e_cd8"];

  
  //Antigenic Exposure and T Cell Activation
  double a_4 = a_0*k_cd4*(cd4/(cd4+x_s));
  double a_8 = a_0*k_cd8*(cd8/(cd8+x_s));
  
  double pk_CD4 = cd4_theta*(k_cd4+1)*pk_cd4[] - (e_cd4 + cd4_theta * k_cd4)*pk_cd4 + e_cd4*p_kcd4; 
  double pk_CD8 = ;
  double epsilon = infectious_active/(infectious_active+ infectious_0);
  double homeostasis = cd4_gamma + cd8_gamma + mu + 2*a_0*(2*p_a + 1);

  // T Cell Dynamics
  double dcd4 = cd4_gamma-a_4 + 2*p_a*mu_a*cd4_activated + mu*cd4 - homeostasis*cd4*(cd4 + cd8);
  double dcd8 = cd8_gamma-a_8 + 2*p_a*mu_a*cd8_activated + mu*cd8 - homeostasis*cd8*(cd4 + cd8);
  double dcd4_activated = a_4 - mu_a*cd4_activated - beta*v*cd4_activated;
  double dcd8_activated = a_8 - mu_a*cd8_activated;
  ...
  NumericVector res_vec = NumericVector::create(dcd4, dcd8, dcd4_activated, dcd8_activated, dinfectious_active, dinfectious_latent, dz, dv);
